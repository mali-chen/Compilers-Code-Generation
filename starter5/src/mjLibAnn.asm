...
#               This is not passed in s2, and methods don't pop parameters
...
###############################################################
    ...
    # The old stack did a weird dance with the $ra and this pointer.
    #
    # Instead we want as simple calling convention as possible
    #
    # foo.method(int a, int b)
    # {
    #     int x = a;
    #     int y = b;
    #     <-- HERE
    ...
    # ---------------
    #
    # The change here is that we push RA onto the stack
    # We don't even lose anything here because in the
    # old system we needed to push the this pointer on the stack.
    #
    # The other two changes are easy, but require a lot of editing here.
    #
    # We return the result in $t0, not on the top of the stack
    # This is more inline with modern calling conventions,
    # and it's easier.
    # 
    # The other change is that we don't pop our parameters.
    # This is more work on the caller's end (not much)
    # but it saves a lot of work on the caller end.
    # More importlantly, it saves a lot of confusion.
    # That stack after a method call is the same
    # as it was before the call.  The answer is just in $t0.
...
goEsFalse:
    ...
    # This is after a return, it can't possibly be executed
    # * bangs head on wall *
...
mth_readLine_Lib:
  ...
  foundNewLine:
    ...
    subu $sp,$sp,4 # push char-count onto stack
    sw $s6,($sp) #**"char count 
...
#   - ($sp) - return address
#   - 4($sp) - k: one beyond index of last char in subrange
#   - 8($sp) - GC tag
#   - 12($sp) - n: index of first char in subrange
#   - 16($sp) - GC tag
#   - 20($sp) - old this pointer
# - return value:
#  - $t0 - substring
########################################################
